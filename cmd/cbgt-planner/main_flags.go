...
func initFlags(flags *Flags) map[string][]string {
	...
	s(&flags.CfgConnect,
		...
		"required connection string to a configuration provider/server"+
			...
			"\n       'export CBAUTH_REVRPC_URL=http://user:password@localhost:9000/cbgt-planner'.")
	b(&flags.DryRun,
	...
	flag.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "%s: couchbase cbgt-planner\n", base)
...
const examples = `
  Example where cbgt's configuration is kept in a couchbase "cfg-bucket":
    ./cbgt-planner -cfg=couchbase:http://cfg-bucket@CB_HOST:8091
`
