...
func initFlags(flags *Flags) map[string][]string {
	...
	s(&flags.CfgConnect,
		...
		"required connection string to a configuration provider/server"+
			...
			"\n       'export CBAUTH_REVRPC_URL=http://user:password@localhost:9000/cbgt-mcp'.")
	b(&flags.DryRun,
	...
	b(&flags.Prompt,
		[]string{"prompt"}, "", false,
		"run in interactive command-line prompt mode.")
	s(&flags.Server,
	...
	i(&flags.WaitForMemberNodes,
		...
		"seconds to wait for member nodes to join at start of replan/rebalance.")
	...
	flag.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "%s: couchbase cbgt-mcp\n", base)
		...
		fmt.Fprintf(os.Stderr, "\nSee also:"+
			" http://github.com/couchbase/cbgt-mcp\n\n")
	}
...
const examples = `
  Example where cbgt's configuration is kept in a couchbase "cfg-bucket":
    ./cbgt-mcp -cfg=couchbase:http://cfg-bucket@CB_HOST:8091
`
